  ...
  try {
    ...
    while (attempts < retryCount) {
      ...
      const response = await fetch(
        "https://api.la5m.ir/servers?platform=" + gameName
      );
      if (!response.ok) throw new Error(response.statusText);
      const getServers = await response.json();

      if (getServers !== 1 && typeof getServers?.data === "object")
        return getServers;
      else {
        await wait(1200);
        $(".app-servers-loading h3").html(
          `سرور مرکزی در حال بارگذاری سرورهای ${
            gameName == "gta5" ? "فایوم" : "رد ام"
          } است<br>لطفاً کمی صبر کنید<span class='loading-tag'>...</span>`
        );
      }
    ...
    $(".app-servers-loading h3").html(
      "اتصال به سرور برای دریافت اطلاعات با شکست مواجه شد!<br>اینترنت خود را بررسی کنید و دوباره تلاش کنید."
    );
  }
}
...
async function UpdateServers() {
  ...
  if (getServers) {
    ...
    for (const server of getServers.data) {
      const licenseTag = server.licenseType
        ? `<img class="app-server-list-server-license-type" src="../assets/img/licenses/${server.licenseType}.svg" alt="License">`
        : "";
      const randomTags = (server.tags?.list || [])
        .sort(() => 0.5 - Math.random())
        .slice(0, Math.floor(Math.random() * 5))
        .map((tag) => `<box><span>${tag}</span></box>`)
        .join("");
      const tags = randomTags
        ? `<main-box class="app-server-list-server-tags hidden lg:flex">${randomTags}</main-box>`
        : "";
      ...
      const serverElement = $(`
                <div id="${server.id}" tags="${
        server.tags?.list
      }" class="app-server-list-main fade-in-row ${
        server.licenseType === "pt" ? "app-server-license-effect" : ""
      }">
                    <div>
                        <img class="app-server-list-server-logo" src="../assets/img/logo-template/${randomLogo}.svg" alt="Server Logo">
                        ${
                          server.offline
                            ? "<offline><span>خاموش</span></offline>"
                            : ""
                        } 
                        <h1 class="app-server-list-server-name">${
                          serverName == "" ? server.hostName : serverName
                        }</h1>
                        <h2 class="app-server-list-server-text">${parseColorCodes(
                          server.projectDescription
                        )}</h2>
                    </div>
                    <div>
                        <span class="app-server-list-server-player-count min-w-[55px] lg:min-w-[80px] [min-inline-size:max-content]">${
                          server.players.maxCount
      ...
      $("<img>")
        ...
        .on("error", () =>
          serverLogo.attr(
            "src",
            `../assets/img/logo-template/${randomLogo}.svg`
          )
        );
    }
